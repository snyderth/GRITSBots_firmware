...
void GRITSBotMain::initialize() {
  ...
  I2CRequestTimeout_ = 4;               /* Wait time for I2C request in ms */
...
/************************
  Methods to hopefully fix I2C hanging
************************/
void GRITSBotMain::ensureI2CBufferInit(){
    for(int i = 0; i < MAX_I2C_MSG_LENGTH; i++){
      I2CBuffer_.data_[i].fval = 0.0;
    }
    I2CBuffer_.msgType_ = 0;
}


void GRITSBotMain::checkI2CHang(){
  int sda = 2, scl = 14;
  pinMode(scl, INPUT);
  pinMode(sda, INPUT);
  if(digitalRead(sda) == LOW && digitalRead(scl) == HIGH){
    // sendStatusMessage("Resetting I2C");
    I2C_Reset();
  }
}


void GRITSBotMain::I2C_Reset(){
  sendStatusMessage("Resetting I2C");
  int sda = 2, scl = 14;
  pinMode(sda, OUTPUT);
...
bool GRITSBotMain::processUDPMessage() {
  ...
  if(JSONGetNumber<uint8_t>(root, String("msgType"), msgType)) {
    switch(msgType) {
      ...
      { /*Case to update firmware from raspberry pi*/
        setVelocities(0.0,0.0);
        t_httpUpdate_return ret = ESPhttpUpdate.update("http://192.168.1.129/newfirmware.bin");
        switch (ret) {
          case HTTP_UPDATE_FAILED:
            sendErrorMessage("Update failed");
            break;

          case HTTP_UPDATE_NO_UPDATES:
            sendStatusMessage("No updates available");
            break;

          case HTTP_UPDATE_OK:
            sendStatusMessage("Update OK");
            break;

          default:
            sendErrorMessage("Unhandled update case");
            break;
        }
      }
      default:
        sendErrorMessage("Unhandled received message");
        return false;
    }
  } else {
...
void GRITSBotMain::setVelocities(float v, float w) {
  ...
  if(I2C_->receiveMessage(&I2CBuffer_)){
    //Serial.println("RPS message received");
    /*sendErrorMessage("Velocity Sent");
    String msgs[2] = {"First num", "Second Num"};
...
bool GRITSBotMain::sampleMotorBoardVelocities() {
  ...
  if(I2C_->receiveMessage(&I2CBuffer_)) {
    ...
    String msgs[2] = {"Linear Velocity", "Angular Velocity"};
    float vals[2] = {v_.fval,w_.fval};
    JSONSendMessage(msgs, vals,2);
    return true;
  } else {
    sendErrorMessage("No response from motor board");
    return false;
  }
}
...
bool GRITSBotMain::sampleMotorBoardRPS() {
  ...
  if(I2C_->receiveMessage(&I2CBuffer_)) {
    ...
    Serial.print("RPS: ");
    Serial.print(rpsLeft_.fval);
    Serial.print(" ");
    Serial.println(rpsRight_.fval);
    return true;
  }
...
bool GRITSBotMain::testMotorBoardI2CCommunications() {
  ...
  if(I2C_->receiveMessage(&I2CBuffer_)) {
    ...
    sendStatusMessage("Reply heard");
    String msgs[2] = {"First num", "Second Num"};
    float vals[2] = {I2CBuffer_.data_[0].fval, I2CBuffer_.data_[1].fval};
    JSONSendMessage(msgs, vals, 2);
    Serial.print("Message float 0: ");
    Serial.println(I2CBuffer_.data_[0].fval);
    Serial.print("Message float 1: ");
    Serial.println(I2CBuffer_.data_[1].fval);
